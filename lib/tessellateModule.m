...
% ARBITRARY SHAPE MODULES, or not defined
elseif ((isfield(probe.module, 'shape')==false) || strcmp(probe.module.shape, 'diamond'))
    ...
    % account for width and height difference in arbitrary shape
    ...
    for row=1:probe.n_modules_y
        for col=1:probe.n_modules_x
            % Find dimensions of the centroid, adjusting for probe.spacing
            x = (maxmodwidth/2) + ((col-1)*maxmodwidth) + ((col-1)*probe.spacing);
            y = (maxmodheight/2) + ((row-1)*maxmodheight) + ((row-1)*probe.spacing);
            %x = (probe.module.dimension/2) + ((col-1)*probe.module.dimension) + ((col-1)*probe.spacing);
            %y = (probe.module.dimension/2) + ((row-1)*probe.module.dimension) + ((row-1)*probe.spacing);
